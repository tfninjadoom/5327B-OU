...
// the move parameter is if moveing forward would help grab the ball for example
if the ball if against a barrier, the same for realeing the ball void
grabBall(int speed, int duration, bool move, int dis) { intake.move(-speed);
  intake2.move(speed);
  if(move){
...
void initialize() {
  ...
  // chassis.set_active_brake(0.1); 	// Sets active brake kP, recommended
  // value 0.1.
  default_constants();        //
...
void autonomous() {
  ...
  chassis.set_drive_brake(MOTOR_BRAKE_COAST);  // Set motors to hold. This helps
                                               // autonomous consistency.

  if (Autonomous::selection == Autonomous::Select::left) {
    Autonomous::left();
  } else if (Autonomous::selection == Autonomous::Select::right) {
    Autonomous::right();
  } else {
    Autonomous::skills();
  }
...
void opcontrol() {
  ...
  while (true) {
    ...
    //                 LCD_BTN_CENTER)
    //                 >>
    //                 1, 				(pros::lcd::read_buttons() & LCD_BTN_RIGHT)
    //                 >> 0);
